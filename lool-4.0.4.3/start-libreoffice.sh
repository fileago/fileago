...
# Updating CA certificates to include FileAgo CA cert
grep fileago.crt /etc/ca-certificates.conf > /dev/null || echo "fileago.crt" >> /etc/ca-certificates.conf
/usr/sbin/update-ca-certificates

# Start loolwsd
su -c "LOOL_NO_AUTOSAVE=true /usr/bin/loolwsd --version --o:sys_template_path=/opt/lool/systemplate --o:lo_template_path=/opt/collaboraoffice6.0 --o:child_root_path=/opt/lool/child-roots --o:file_server_root_path=/usr/share/loolwsd ${extra_params}" -s /bin/bash lool
